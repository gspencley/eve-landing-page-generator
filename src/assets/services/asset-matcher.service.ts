...
export class AssetMatcherService {
  matchAssets(profile: FirmProfile): AssetMatchResult {
    const scored = ASSET_LIBRARY.map((asset) => this.scoreAsset(asset, profile)).sort(
  ...
  private scoreAsset(asset: MarketingAsset, profile: FirmProfile): AssetScoreBreakdown {
    let totalScore = asset.baseWeight;
    const reasons: string[] = [`Base relevance weight: ${asset.baseWeight}`];
    const penalties: string[] = [];

    totalScore += this.matchList(
